        ...
        <AnimatePresence>
            {isScrolling ? (
                <motion.nav key={1}
                    ...
                    animate="animate"
                    exit="exit"
                    className="m-auto md:fixed z-[9999] right-2 left-0 px-6 py-3 text-white bg-gray-400/40 top-10 rounded-3xl backdrop:blur w-fit">

                        <div className="items-center hidden gap-5 md:flex">
                                {dataHeader.map(({id, name, link}) => (
                                    <Link key={id} href={link} className="hover:text-secondary hover:border-b-[1px]">{name}</Link>
                                ))}
